...
const flashSwdSh = join(root, "domains/glyf/display/flash-swd.sh");
...
async function runBuildFlashLaunch(flashMode) {
  ...
  if (flashMode === "swd") {
    await runScript(flashSwdSh, []);
  } else if (flashMode === "picotool") {
...
function actionOptions() {
  return [
    ...
    {
      value: "flash-swd",
      label: "Flash Existing ELF via SWD",
      hint: "preferred Debug Probe / OpenOCD path",
    },
    {
      value: "flash-picotool",
      label: "Flash Existing UF2 via picotool",
      hint: "explicit USB tool path",
    },
    {
      value: "flash-uf2",
      label: "Flash Existing UF2 via mounted RPI-RP2",
      hint: "explicit BOOTSEL mass-storage path",
    },
    {
      value: "launch-app",
      label: "Launch Glyf App",
      hint: "start the Tauri companion only",
    },
    {
      value: "workflow-swd",
      label: "Build -> Flash via SWD -> Launch App",
      hint: "recommended daily dev loop",
    },
    {
      value: "workflow-picotool",
      label: "Build -> Flash via picotool -> Launch App",
      hint: "full dev loop",
    },
    {
      value: "workflow-uf2",
      label: "Build -> Flash via RPI-RP2 -> Launch App",
      hint: "bring-up / recovery loop",
    },
...
async function main() {
  ...
  try {
    ...
    else if (action === "flash-swd") await runScript(flashSwdSh, []);
    ...
    else if (action === "workflow-swd") await runBuildFlashLaunch("swd");
    else if (action === "workflow-picotool")
      await runBuildFlashLaunch("picotool");
